...
AIRPORT_COORDS = {
    'MAN': (53.3537, -2.2750), 'LGW': (51.1537, -0.1821), 'LHR': (51.4700, -0.4543),
    'EDI': (55.9500, -3.3725), 'BRS': (51.3827, -2.7190), 'LPL': (53.3337, -2.8497),
    'BHX': (52.4539, -1.7480), 'NCL': (55.0375, -1.6917), 'GLA': (55.8719, -4.4333),
    'AMS': (52.3105, 4.7683), 'CDG': (49.0097, 2.5479), 'FCO': (41.8003, 12.2389),
    'MAD': (40.4983, -3.5676), 'BCN': (41.2974, 2.0833), 'DUB': (53.4213, -6.2701),
    'BRU': (50.9010, 4.4856), 'CPH': (55.6180, 12.6508), 'ARN': (59.6519, 17.9186),
    'OSL': (60.1939, 11.1004), 'VIE': (48.1103, 16.5697), 'ZRH': (47.4647, 8.5492),
    'PRG': (50.1008, 14.2632), 'ATH': (37.9364, 23.9445), 'LIS': (38.7742, -9.1342),
    'AGP': (36.6749, -4.4991), 'PMI': (39.5517, 2.7388), 'FAO': (37.0144, -7.9659),
    'NCE': (43.6584, 7.2159), 'VCE': (45.5053, 12.3519), 'MXP': (45.6306, 8.7281),
}
...
def classify_route_type(distance_km: int) -> str:
    ...
    if distance_km < 500: return "domestic"
    elif distance_km < 1500: return "european_short"
    elif distance_km < 3000: return "european_long"
    else: return "intercontinental"
...
def load_crisis_config() -> List[CrisisEvent]:
    ...
    if not os.path.exists(config_path):
        print(f"⚠️  Crisis config not found: {config_path} - continuing without crisis flags")
        return []
...
def check_crisis_flags(snapshot_date: dt.date, destination_iata: str, crisis_events: List[CrisisEvent]) -> Dict[str, Any]:
    ...
    flags = {
        "crisis_flag": False, "crisis_id": None, "crisis_severity": None,
        "crisis_route_affected": False, "crisis_global_impact": False,
        "crisis_contamination_pct_t14": 0.0, "crisis_contamination_pct_t7": 0.0,
        "crisis_label_contaminated": False, "training_action": None,
    }
...
def shi_variance_calculation(supabase: Client, origin: str, dest: str, outbound_date: dt.date, 
                             return_date: dt.date, current_price: float) -> Tuple[str, Optional[float]]:
    """Calculate SHI z-score. Returns (flag, z_score)."""
    try:
        result = supabase.table('snapshots').select('price_gbp').eq(
            'origin_iata', origin
        ).eq('destination_iata', dest).eq('outbound_date', str(outbound_date)).eq(
...
def search_duffel(origin: str, dest: str, outbound: dt.date, return_date: dt.date, 
                 cabin_class: str, duffel_token: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        return {
            ...
            "lcc_present": any(
                seg["marketing_carrier"].get("name", "").lower() in ["ryanair", "easyjet", "wizz air", "norwegian"]
                for slice_ in cheapest["slices"]
                for seg in slice_["segments"]
            ),
...
def main():
    ...
    destinations = ["AMS", "CDG", "BCN", "DUB", "FCO", "MAD", "ATH", "LIS", "AGP", "PMI", 
    ...
    for origin, dest, outbound, return_date, cabin in routes:
        ...
        row = {
            "snapshot_id": snapshot_id, "snapshot_date": str(snapshot_date),
            "capture_time_utc": capture_time, "origin_iata": origin, "destination_iata": dest,
            ...
            "is_school_holiday_window": False, "is_bank_holiday_adjacent": False,
            ...
            "cabin_class": cabin, "seats_remaining": None,
            "price_t7": None, "price_t14": None, "rose_10pct": None, "fell_10pct": None,
            "snapshot_key": snapshot_key, "notes": None, "origin_type": "Tier1",
            ...
            **crisis_flags,
            ...
            "route_distance_km": distance_km, "route_type": route_type,
            "shi_score": shi_score, "model_version": "v1_0_0"
        }
    ...
    print(f"\n{'=' * 70}\n✅ Capture complete: {len(snapshots)} snapshots\n{'=' * 70}")
