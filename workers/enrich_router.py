...
# - Fast: single get_all_values() per sheet.
...
def stable_pick(items: List[Dict[str, Any]], seed: str) -> Optional[Dict[str, Any]]:
    if not items:
        return None
    # deterministic selection: hash seed -> index
    import hashlib
...
def main() -> int:
    ...
    # Eligible statuses (V5)
    ELIGIBLE = {"PUBLISH_AM", "PUBLISH_PM", "PUBLISH_BOTH", "SCORED", "READY_TO_POST", "READY_TO_PUBLISH"}
    ...
    try:
        ...
        if vals and len(vals) >= 2:
            ...
            if i_code is not None and i_city is not None and i_country is not None:
                for r in vals[1:]:
    ...
    try:
        ...
        if vals and len(vals) >= 2:
            ...
            if None in (i_dest, i_theme, i_cat, i_phrase, i_appr):
                raise RuntimeError("PHRASE_BANK missing required headers")
    ...
    # Usage index (month-aware) from RAW_DEALS itself (counts phrase_used)
    usage: Dict[str, int] = {}
    ...
    if c_phrase_used is not None:
        for r in raw[1:]:
            ...
            ts = ""
            # prefer posted_vip_at -> posted_free_at -> posted_instagram_at
            for col in ("posted_vip_at", "posted_free_at", "posted_instagram_at"):
                ...
                if ci is not None and ci < len(r):
                    ...
                    if ts:
                        break
            if not phrase or not ts:
                continue
            # month filter: accept if timestamp begins with YYYY-MM (ISO)
    ...
    def channel_ok(p: Dict[str, Any]) -> bool:
        ...
        # if channel_hint is descriptive (e.g. "Destination-specific") treat as not a filter
    ...
    for row_num, r in enumerate(raw[1:], start=2):
        ...
        if iata_map:
            if c_origin_city is not None and not getv(r, c_origin_city) and origin in iata_map:
                ...
                if city:
                    queue(row_num, c_origin_city + 1, city)
                    cityfills += 1
                    row_changed = True
            ...
            if c_dest_city is not None and not getv(r, c_dest_city) and dest in iata_map:
                ...
                if city:
                    queue(row_num, c_dest_city + 1, city)
                    cityfills += 1
                    row_changed = True
            ...
            if c_dest_country is not None and not getv(r, c_dest_country) and dest in iata_map:
                ...
                if country:
                    queue(row_num, c_dest_country + 1, country)
                    cityfills += 1
                    row_changed = True
        ...
        if c_phrase_used is not None and not getv(r, c_phrase_used):
            ...
            if chosen:
                ...
                if dest and theme:
                    phrase_misses += 1
                    if REQUIRE_PHRASE:
                        # leave blank; downstream can gate
                        pass
    ...
    log(f"Rows enriched: {enriched} (cap {MAX_ROWS})")
