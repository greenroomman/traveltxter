...
✅ Filters using raw_status == READY_TO_POST
...
✅ Promotes raw_status -> POSTED_INSTAGRAM
...
def post_to_instagram(image_url: str, caption: str) -> Tuple[bool, str]:
    ...
    logger.info(f"   Creating Instagram container...")
    ...
    logger.info(f"   Publishing container...")
    ...
    try:
        ...
        for row_num, row in enumerate(rows, start=2):
            ...
            if ig_status in ("POSTED", "PUBLISHED", "DONE"):
                logger.debug(f"Row {row_num}: Already posted, skipping")
                continue
            ...
            logger.info(f"   Image: {deal['graphic_url']}")
        ...
        logger.info("📊 Stats saved to logs/publish_stats.json")
