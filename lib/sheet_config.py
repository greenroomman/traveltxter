...
def _read_tab_as_dicts(sh: gspread.Spreadsheet, tab_name: str) -> List[Dict[str, str]]:
    """
    Reads entire tab into list[dict] using header row. Missing tab => [].
    """
    try:
...
def load_config_bundle(sh: gspread.Spreadsheet) -> ConfigBundle:
    """
    Loads all control-plane tabs. Missing tabs return empty lists.
    """
    return ConfigBundle(
...
def pick_theme_for_today(themes_rows: List[Dict[str, str]], override: str = "") -> str:
    ...
    Uses the set of theme names found in THEMES tab.
    """
    if override:
...
def active_config_routes(config_rows: List[Dict[str, str]], theme: str) -> List[Dict[str, str]]:
    """
    Filters CONFIG rows by enabled==TRUE and matching theme.
    ...
    out.sort(
        key=lambda x: (_to_int(x.get("priority", "0"), 0), x.get("origin_iata", ""), x.get("destination_iata", "")),
        reverse=True,
    )
    return out
...
def origins_for_today(origin_rows: List[Dict[str, str]]) -> List[str]:
    ...
    Chooses an origin_pool based on day-of-year, returns origin_iata list by priority.
    """
    if not origin_rows:
        return []
    ...
    rows = [
        ...
        if r.get("origin_pool", "").strip() == pool and r.get("origin_iata", "").strip()
    ]
...
def theme_destinations(themes_rows: List[Dict[str, str]], theme: str, limit: int = 20) -> List[str]:
    """
    Returns destination_iata list for a theme, ordered by priority desc.
    """
    t = _norm(theme)
    rows = [
        r for r in themes_rows
        if _norm(r.get("theme", "")) == t and r.get("destination_iata", "").strip()
    ]
    rows.sort(key=lambda x: _to_int(x.get("priority", "0"), 0), reverse=True)
...
def mvp_hard_limits(_mvp_rows: List[Dict[str, str]]) -> Dict[str, Any]:
    """
    Safe, deterministic hard limits used across workers.
    (We can extend later to interpret MVP_RULES more deeply.)
    """
    return {
        "min_price_gbp": 10.0,
        "max_price_gbp": 800.0,
...
def carrier_bias_weight(bias_rows: List[Dict[str, str]], theme: str, destination_iata: str, carrier_code: str) -> float:
    """
    Optional helper: returns bias_weight when theme+dest+carrier match.
    """
    t = _norm(theme)
