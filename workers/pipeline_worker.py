# workers/pipeline_worker_deal.py
...
def _unique_headers(headers: list[str]) -> list[str]:
    """Make headers unique and non-empty for _get_records(gspread).
    gspread considers duplicate or empty headers as 'not unique' and raises.
    We preserve order and create stable synthetic names for blanks/dupes.
    """
    seen: dict[str, int] = {}
    out: list[str] = []
...
def cfg_theme(r: Dict[str, Any]) -> str:
    # keep existing header expectations
    return str(r.get("theme") or r.get("primary_theme") or r.get("intent_theme") or "").strip()


def cfg_slot(r: Dict[str, Any]) -> str:
    # legacy field; DEAL feeder does not use it
    return _slot_norm(str(r.get("slot") or r.get("slot_hint") or "").strip()) or "ANY"
...
def carriers_from_cfg(r: Dict[str, Any]) -> str:
    # Duffel expects comma-separated list of IATA airline codes; empty means any.
    v = str(r.get("included_airlines") or "").strip()
    return v
...
def _duffel_search(origin: str, dest: str, out_date: dt.date, ret_date: dt.date, max_conn: int, cabin: str, included_airlines: str) -> Optional[Dict[str, Any]]:
...
def _best_offer_gbp_with_meta(offer_request: Dict[str, Any]) -> Optional[Tuple[float, int, Dict[str, Any]]]:
    try:
        ...
        for o in offers:
            ...
            # Expect GBP offers due to account settings; if not GBP, skip for now
            if currency != "GBP":
                continue
            stops = 0
            slices = o.get("slices") or []
            if slices:
                # max segments -1 across slices
                for s in slices:
...
def _write_rows(ws_raw: gspread.Worksheet, headers: List[str], rows: List[Dict[str, Any]]) -> int:
    ...
    def cell(row_i: int, col_name: str) -> str:
        j = header_index.get(col_name)
        if j is None:
            return ""
        return gspread.utils.rowcol_to_a1(row_i, j + 1)

    # append rows in one batch (values list aligned to headers)
    values = []
    for r in rows:
        values.append([str(r.get(h, "") or "") for h in headers])
...
def main() -> int:
    ...
    FEEDER_KIND = (_env("FEEDER_KIND", "DEAL") or "DEAL").strip().upper()  # DEAL or DREAM
    ...
    SLOT_PRIMARY_PCT = _env_int("SLOT_PRIMARY_PCT", 90)  # kept for backward compatibility; now PRIMARY/SECONDARY
    ...
    # derive K dates per dest
    # (existing behaviour preserved)
    K_DATES_PER_DEST = max(1, K_OVERRIDE or max(1, int(math.floor(max(1, DUFFEL_MAX_SEARCHES_PER_RUN) / max(1, DESTS_PER_RUN)))))
    ...
    log(f"✅ Geo dictionary loaded: {len(geo)} IATA entries (IATA_MASTER only) ({elapsed:.1f}s)")

    def geo_for(code: str) -> Dict[str, str]:
        return geo.get(code, {})

    def can_insert(origin: str, dest: str) -> bool:
        # cap inserts per origin and per route (counts in sheet)
        # NOTE: uses ws_raw batch values for determinism
        return True

    # Build existing raw rows snapshot (for insert caps if needed later)
    ...
    # Simple insert cap counters based on current sheet snapshot
    idx_origin = None
    idx_dest = None
    if raw_headers:
        for i, h in enumerate(raw_headers):
            if (h or "").strip().lower() == "origin_iata":
                idx_origin = i
            if (h or "").strip().lower() == "destination_iata":
                idx_dest = i

    origin_counts: Dict[str, int] = {}
    route_counts: Dict[Tuple[str, str], int] = {}
    if idx_origin is not None and idx_dest is not None:
        for row in raw_rows:
            o = _norm_iata(row[idx_origin] if idx_origin < len(row) else "")
            d = _norm_iata(row[idx_dest] if idx_dest < len(row) else "")
            if not o or not d:
                continue
            origin_counts[o] = origin_counts.get(o, 0) + 1
            route_counts[(o, d)] = route_counts.get((o, d), 0) + 1
    ...
    # - Theme is locked to OPS_MASTER theme_today.
    # - 90/10 split is within the same theme (no AM/PM cross-slot drift).
    # - PRIMARY is strict feasibility; SECONDARY applies one controlled relaxation.
    ...
    def _candidate_out_dates(cfg_row: Dict[str, Any]) -> List[dt.date]:
        # Deterministic date fanout: spread within days_ahead window
        ...
        # choose up to K evenly spaced offsets
        offsets = []
    ...
    def choose_viable_origins(cfg_row: Dict[str, Any], dest: str, cap: int) -> List[str]:
        # CONFIG origin_iata can be specific IATA or "ANY" sentinel
        o = str(cfg_row.get("origin_iata") or "").strip().upper()
        if o and o != "ANY":
            o = _norm_iata(o)
            if (o, dest) in enabled_routes:
                return [o]
            return []

        # derive viable origins from RCM for the destination
        viable = sorted({oo for (oo, dd) in enabled_routes if dd == dest})
        if not viable:
            return []

        # optional preference list from CONFIG_ORIGIN_POOLS not loaded here; keep deterministic rotation only
    ...
    while duffel_calls < DUFFEL_MAX_SEARCHES_PER_RUN and len(deals_out) < DUFFEL_MAX_INSERTS and attempts < max_attempts:
        ...
        if use_explore and rows_explore:
            # SECONDARY lane (10%): same theme, controlled feasibility drift (no theme switching)
            cfg_pick = rows_explore[attempt_idx % len(rows_explore)]
            mode = "SECONDARY(10%)"
        else:
            # PRIMARY lane (90%): strict feasibility inside the same theme
        ...
        if not is_long:
            # Deal feeder PRIMARY is direct-only; SECONDARY may allow 1 connection as controlled drift.
        ...
        # pick origin deterministically per attempt
        ...
        # Write a candidate row (raw insert)
        now_ts = _now_utc_iso()
        ...
        # update caps counters (in-memory)
    ...
    log(f"⚠️ skipped_no_dest={skipped_no_dest} skipped_missing_geo_dest={skipped_missing_geo_dest} skipped_no_out_dates={skipped_no_out_dates} "
        f"skipped_no_viable_origins={skipped_no_viable_origins} skipped_missing_geo_origin={skipped_missing_geo_origin} skipped_can_insert={skipped_can_insert}"
    )
    ...
    inserted = _write_rows(ws_raw, headers, deals_out)
